        ...
        with open(fname) as f:
                ...
                for line in f:
                        if line1=='True':
                        	...
                        	#print(len(content))
                        	content=np.reshape(np.fromstring(line, dtype=int,count=768, sep=' '), (256,3))
                        	content_array.append(content)
                        	#print(content_array)

            	content_array=np.asarray(content_array)
                temp=np.all(content_array==1,axis=2)
                content_array[temp]=0
...
# ###############                           Display setup  original symmetric          #############################
#     fig = plt.figure()
...
#     normalize = Normalize(vmin=1400, vmax=1800)
#     area = np.ma.masked_where(abs_img_fft < 400, abs_img_fft)
#     area=area/10
